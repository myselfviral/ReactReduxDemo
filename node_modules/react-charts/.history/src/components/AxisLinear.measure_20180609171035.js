...
export default function measure () {
  ...
  if (isHorizontal) {
    ...
    const leftWidth = getPixel(labelDims[0].width)
    const rightWidth = getPixel(labelDims[labelDims.length - 1].width)
    if (resolvedRotation) {
      right = Math.ceil(fontSize / 2)
      left =
        axis.stepSize / 2 - Math.abs(Math.ceil(Math.cos(resolvedRotation) * leftWidth)) + fontSize
      console.log(axis.stepSize, resolvedRotation, leftWidth)
    } else {
  ...
  dispatch(state => {
    const axisDimensions = state.axisDimensions
    const position = axisDimensions[position] || {}
    return {
      ...state,
      axisDimensions: {
        ...axisDimensions,
