...
function defaultRenderer (props) {
  ...
  const start = activeIndex > -1 ? activeIndex - resolvedShowCount / 2 : 0
  const end = activeIndex > -1 ? start + resolvedShowCount / 2 : sortedGroupDatums.length
  const visibleSortedGroupDatums = sortedGroupDatums.slice(Math.max(start, 0), end)
  const hasPrevious = start > 0
  const hasNext = end < sortedGroupDatums.length
