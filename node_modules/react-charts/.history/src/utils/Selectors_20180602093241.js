...
export default {
  ...
  gridX: () =>
    Memoize(
      state => [(state && state.padding && state.padding.left) || 0,
        state && state.axesDimensions && axesDimensions.left
        state && state.axesDimensions && axesDimensions.top
        state && state.axesDimensions && axesDimensions.bottom
      ],
      (paddingLeft, axesDimensionsLeft, axesDimensionsTop, axesDimensionsBottom) => {
        const axesLeftWidth =
          (axesDimensionsLeft && sumObjBy(axesDimensionsLeft, 'width')) || 0
        const axesTopLeft =
          (axesDimensionsTop && sumObjBy(axesDimensionsTop, 'left')) || 0
        const axesBottomLeft =
          (axesDimensionsBottom && sumObjBy(axesDimensionsBottom, 'left')) || 0
  ...
  gridY: () =>
    Memoize(
      state => [(state && state.padding && state.padding.top) || 0,
        state && state.axesDimensions && axesDimensions.top
        state && state.axesDimensions && axesDimensions.left
        state && state.axesDimensions && axesDimensions.right
      ],
      (paddingTop, axesDimensionsTop, axesDimensionsLeft, axesDimensionsRight) => {
        const axesTopHeight =
          (axesDimensionsTop && sumObjBy(axesDimensionsTop, 'height')) || 0
        const axesLeftTop =
          (axesDimensionsLeft && sumObjBy(axesDimensionsLeft, 'top')) || 0
        const axesRightTop =
          (axesDimensionsRight && sumObjBy(axesDimensionsRight, 'top')) || 0
