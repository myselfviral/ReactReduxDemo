...
import Api from '../apis/SubscriberApi';
import * as types from '../../actions/actionConstants';
import { anchorTable } from '../../containers/Tables/demos/AnchorTable/SubscriberData';
const branch = 'subscriber';
// const branch = 'crudTbFrmDemo';

export function* watchFetchDatas(action) {
  yield put(fetchActionStarted(action.branch));
  try {
    const data = yield Api.getSubscribers().then(response => response.json());
    yield put(fetchActionSuccess(data.subscribers, action.branch));
  } catch (e) {    
