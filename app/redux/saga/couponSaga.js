...
import Api from '../apis/CouponApi';
import * as types from '../../actions/actionConstants';
import { anchorTable } from '../../containers/Tables/demos/AnchorTable/CouponData';
...
export function* watchFetchDatas(action) {
  ...
  try {
    const data = yield Api.getCoupon().then(response => response.json());
    yield put(fetchActionSuccess(data.coupons, action.branch));
  } catch (e) {    
...
export function* watchUpdateData(action) {
  ...
  try {
    if (arr.cg_id === '' || arr.cg_id === 0 || arr.cg_id === undefined ) {
      const response = yield call(Api.addCoupon, arr);
      if (response) {
        yield put(submitActionSuccess(response.data.coupons, action.branch));
      }
    } else {
      const response = yield call(Api.updateCoupon, arr);
      if (response) {
